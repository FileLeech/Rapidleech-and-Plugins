...
class youtube_com extends DownloadClass {
        ...
        public function Download($link) {
                ...
                $this->vid = $this->vid[1];
                $this->link = 'http://www.youtube.com/watch?v='.$this->vid;
        ...
        private function getCipher() {
                ...
                if (@file_exists($savefile) && ($file = file_get_contents($savefile, NULL, NULL, -1, 822)) && ($saved = @unserialize($file)) && is_array($saved) && !empty($saved['sts']) && $saved['sts'] == $this->sts && !empty($saved['steps']) && preg_match('@^\s*([ws]\d+|r)( ([ws]\d+|r))*\s*$@', $saved['steps'])) {
                        ...
                        $this->playerJs = $this->GetPage('http://s.ytimg.com/yts/jsbin/'.$this->js[0], $this->cookie, 0, 'http://www.youtube.com/embed/'.$this->vid);
...
// [12-8-2014]  Forced https on some requests for avoid some errors. - Th3-822
// [09-12-2014] Little Touch in Plugin by Tblogger
?>
